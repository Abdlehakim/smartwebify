...
/* ===================== NiceSelect (Orders/Products style) ===================== */
  ...
  return (
    <div className="mx-auto px-2 py-4 w-[95%] flex flex-col gap-4 h-full bg-green-50 rounded-xl">
      ...
      <div className="flex h-16 justify-between items-start">
        <h1 className="text-3xl font-bold uppercase">Sous-catégories</h1>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Sticky header (combined cols like Categories) */}
          ...
          <table className="table-fixed w-full">
            {displayed.length === 0 && !loading ? (
              ...
              <tbody className="divide-y divide-gray-200 [&>tr]:h-12">
                {displayed.map((sc, i) => (
                  <tr key={sc._id} className={i % 2 ? "bg-green-50" : "bg-white"}>
                    <td className="px-4 text-center">{sc.reference}</td>
                    <td className="px-4 text-center font-semibold truncate">{sc.name}</td>

                    {/* Combined author */}
                    <td className="px-4 text-center max-2xl:hidden">
                      {fmtUser(sc.updatedBy) || fmtUser(sc.createdBy)}
                    </td>

                    {/* Combined date */}
                    <td className="px-4 text-center max-2xl:hidden">
                      {fmtDate(sc.updatedAt || sc.createdAt)}
                    </td>
                    ...
                    <td className="px-4 text-center">
                      <div className="flex justify-center items-center gap-2 h-8">
                        ...
                        <NiceSelect<Status>
                          ...
                          onChange={(v) => updateStatus(sc._id, v)}
                          className="h-8 py-0 leading-8 truncate"
                          disabled={!!saving[sc._id]}
                          loading={!!saving[sc._id]}
                        />

                        <Link href={`/dashboard/manage-stock/sub-categories/voir/${sc._id}`}>
                          <button className="ButtonSquare h-8" disabled={!!saving[sc._id]}>
                            <FaRegEye size={14} />
                          </button>
                        </Link>
      ...
      {/* Mobile cards (already show combined meta) */}
              ...
              >
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="font-semibold leading-tight truncate">{sc.name}</div>
                    <div className="text-xs text-gray-500 mt-0.5">Réf : {sc.reference}</div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <Link href={`/dashboard/manage-stock/sub-categories/voir/${sc._id}`}>
                      <button className="ButtonSquare" disabled={!!saving[sc._id]}>
                        <FaRegEye size={14} />
                      </button>
                    </Link>
                    <Link href={`/dashboard/manage-stock/sub-categories/update/${sc._id}`}>
                ...
                <div className="mt-2 text-xs text-gray-600 flex items-center justify-between">
                  <span>{sc.updatedBy?.username || sc.createdBy?.username || "—"}</span>
                  <span>{fmtDate(sc.updatedAt || sc.createdAt)}</span>
                </div>
