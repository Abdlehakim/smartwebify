...
export default function CreateSubCategoryPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { categories } = await fetchFromAPI<{ categories: Category[] }>("/dashboardadmin/stock/categories");
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      await fetchFromAPI<{ message: string }>("/dashboardadmin/stock/subcategories/create", {
        method: "POST",
        body: fd,
      });
  ...
  return (
    <div className="w-[80%] mx-auto flex flex-col gap-6 p-4 relative h-full">
      <div className="flex flex-col gap-2">
        <h1 className="text-3xl font-bold">Créer une sous-catégorie</h1>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-8">
        ...
        <div className="flex max-lg:flex-col w-full gap-4">
          ...
          >
            ...
            {iconFile ? (
              <div className="relative w-full h-full rounded overflow-hidden">
                <Image src={URL.createObjectURL(iconFile)} alt="Aperçu icône" fill className="object-cover" />
                <button
                  type="button"
          ...
          >
            ...
            {imageFile ? (
              <div className="relative w-full h-full rounded overflow-hidden">
                <Image src={URL.createObjectURL(imageFile)} alt="Aperçu image" fill className="object-cover" />
                <button
                  type="button"
          ...
          >
            ...
            {bannerFile ? (
              <div className="relative w-full h-full rounded overflow-hidden">
                <Image src={URL.createObjectURL(bannerFile)} alt="Aperçu bannière" fill className="object-cover" />
                <button
                  type="button"
        ...
        <div className="flex justify-center gap-8">
          ...
          <button type="submit" disabled={submitting} className="px-6 py-2 bg-tertiary text-white rounded">
            {submitting ? "Ajout en cours…" : "Ajouter la sous-catégorie"}
          </button>
        </div>
      </form>
