// ------------------------------------------------------------------
// src/app/dashboard/blog/postsubcategorie/page.tsx
// ------------------------------------------------------------------
"use client";
...
/* ===================== Page ===================== */
export default function PostSubCategoriesClientPage() {
  const [items, setItems] = useState<PostSubCategorie[]>([]);
  const [loading, setLoading] = useState<boolean>(true);

  const [searchTerm, setSearchTerm] = useState<string>("");
  const [currentPage, setCurrentPage] = useState<number>(1);

  // Désactivation des contrôles pendant sauvegarde par ligne
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const result = await fetchFromAPI<{ PostSubCategories: PostSubCategorie[] }>(
          "/dashboardadmin/blog/postsubcategorie"
        );
        setItems(result.PostSubCategories);
      } catch (err) {
        console.error("Failed to load post sub-categories:", err);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const confirmDelete = async (id: string) => {
    setIsDeleting(true);
    try {
      await fetchFromAPI<void>(`/dashboardadmin/blog/postsubcategorie/delete/${id}`, {
        method: "DELETE",
      });
      setItems((prev) => prev.filter((c) => c._id !== id));
    } catch (err) {
      console.error("Delete failed:", err);
      alert("Impossible de supprimer la sous-catégorie d’article.");
    } finally {
      setIsDeleting(false);
  ...
  const updateStatus = async (id: string, newStatus: Status) => {
    ...
    try {
      await fetchFromAPI<void>(`/dashboardadmin/blog/postsubcategorie/update/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ vadmin: newStatus }),
      });
    } catch {
      setItems((prev) => [...prev]); // simple re-render (pour revert exact, conserver l’ancien)
  ...
  return (
    <div className="mx-auto px-2 py-4 w-[95%] flex flex-col gap-4 h-full bg-green-50 rounded-xl">
      ...
      <div className="flex h-16 justify-between items-start">
        <h1 className="text-3xl font-bold uppercase">Sous-catégories d’articles</h1>
        <Link href="/dashboard/blog/postsubcategorie/create">
          <button className="btn-fit-white-outline">Créer une sous-catégorie</button>
        </Link>
      </div>
                  ...
                  <tr key={c._id} className={i % 2 ? "bg-green-50" : "bg-white"}>
                    ...
                    <td className="px-4 text-center">
                      <div className="flex justify-center items-center gap-2 h-8">
                        ...
                        <Link href={`/dashboard/blog/postsubcategorie/voir/${c._id}`}>
                          <button className="ButtonSquare h-8" disabled={!!saving[c._id]}>
                            <FaRegEye size={14} />
                          </button>
                        </Link>
                        <Link href={`/dashboard/blog/postsubcategorie/update/${c._id}`}>
                          <button className="ButtonSquare h-8" disabled={!!saving[c._id]}>
                            <FaRegEdit size={14} />
                          </button>
                        </Link>
                        <button
                          ...
                          aria-label="Supprimer la sous-catégorie d’article"
                          disabled={!!saving[c._id]}
                        >
                          <FaTrashAlt size={14} />
                        </button>
      ...
      {/* Cartes mobiles (optionnel, même style) */}
      <div className="md:hidden w-full">
        {displayed.length === 0 && !loading ? (
          <div className="py-6 text-center text-gray-600">Aucune sous-catégorie trouvée.</div>
        ) : (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-500 mt-0.5">Réf : {c.reference}</div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <Link href={`/dashboard/blog/postsubcategorie/voir/${c._id}`}>
                      <button className="ButtonSquare" disabled={!!saving[c._id]}>
                        <FaRegEye size={14} />
                      </button>
                    </Link>
                    <Link href={`/dashboard/blog/postsubcategorie/update/${c._id}`}>
                ...
                <div className="mt-2 text-xs text-gray-600 flex items-center justify-between">
                  <span>{c.updatedBy?.username || c.createdBy?.username || "—"}</span>
